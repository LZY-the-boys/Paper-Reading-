...
- Background Knowledge
    - **Formal Proof vs. Informal Proof**:
        - A formal proof is something that can be automatically verified using an Interactive Theorem Prover (ITP), whereas an informal proof can only be verified by a human.
    - **Monte Carlo Tree Search (MCTS)**:
        - **UCT / UCB1**:
            - $$\underbrace{\frac{w_i}{n_i}}_{\text{exploitation: higher win ratio}} + \underbrace{c \sqrt{\frac{\ln N_i}{n_i}}}_{\text{exploration: more simulation}}$$
        ...
        - In DeepSeek Prover:
            - $$w_i = \sum_\tau R(\tau)$$, $$n_i = |\tau|$$, where $$\tau$$ is a list of tree-policy trajectories.
            - The DUCB adds $$\gamma$$ to $$w_i$$ and $$n_i$$.
        - Code:
            - [ReST-MCTS](https://github.com/THUDM/ReST-MCTS)
            - [openr](https://github.dev/openreasoner/openr)
            - [DeepSeek-Prover-V1.5](https://github.com/deepseek-ai/DeepSeek-Prover-V1.5)
    - **Lean**:
        - Supports two theorem proving modes:
            - **Term Mode**: Expresses theorems using ordinary function composition syntax.
            - **Tactic Mode**: Uses a series of commands to invoke automated proof tools interactively. This is the mode primarily used by DeepSeek.
            - The `by` keyword introduces tactic mode, allowing users to build proofs step-by-step using various tactics like `cases`, `intro`, `apply`, etc.

-   [DeepSeek-Prover-V1.5](https://www.arxiv.org/abs/2408.08152)
    1. **Evaluation**:
        - **Metrics**: ProofNet test and miniF2F-test, ACC under controlled sample budget.
    2. **Inference**:
        - **Hybrid Approach**: Combines Single-pass whole-proof generation and Multi-pass proof-step generation.
        - **Truncate-and-Resume Mechanism**: Generates a complete proof first, then truncates and resumes when errors are found. Finds a successful initial segment and parses it into individual tactics, starting MCTS from this point.
    3. **Training**:
        - **Pretraining**: Continues on DeepSeekMath, adding Lean, Isabelle, and Metamath.
        - **Supervised Fine-Tuning (SFT)**:
            - **Data**: Mathlib, theorems generated by DeepSeekProver-V1, Lean Workbook, and validation sets from miniF2F and ProofNet benchmarks.
            - **CoT Method**: Inserts natural language thinking processes as comments before each proof step. Combines CoT and non-CoT approaches.
            - **Intermediate Tactic State Information**: Adds comments in Lean 4 code to support truncate-and-resume mechanism, e.g., `/- tactic state: ... -/`.
        - **MCTS RL Training Loss (RMaxTS)**:
            - **Data**: 4.5k filtered examples from SFT data (CoT & non-CoT mix).
            - **TreePolicy**: $$TreePolicy = \text{argmax } Q_{UCB}(s,a)$$, trained with GRPO.
            - **Sparse Rewards**: Adds an exploration reward (RMax) to encourage exploring new nodes.
            - **Discounted UCB**: Uses discounted UCB to give more weight to recent rewards and reduce the impact of outdated information.

- [DeepSeek-Prover](https://arxiv.org/abs/2405.14333)
    1. **Data**: Collects and synthesizes data from high-school and undergraduate math competition problems (8M MMA dataset)
        - **SFT on DeepSeekMath-Base**: Trains for Whole-Proof Generation.
    2. **Benchmark**:
        - **miniF2F test** and **FIMO bench**.

-   Other Baselines
    1. [**Lean-STaR**](https://arxiv.org/abs/2407.10040): Adds informal thoughts before each proof step (CoT).
    2. [**[COLM24] COPRA**](https://arxiv.org/abs/2310.04353): Introduces an agent workflow, repeatedly calling GPT-4 in a stateful backtracking search.
        -   https://openreview.net/forum?id=XCMbagV0No 审稿意见值得学习
    3. [**[NIPS23] ReProver**](https://arxiv.org/abs/2306.15626): Uses retrieval-augmented language models with LeanDojo.
    4. [**[OpenAI] GPT-f**](https://arxiv.org/abs/2009.03393): Generative Language Modeling for Automated Theorem Proving.
    5. [**[ICLR24] LLemma**](https://arxiv.org/abs/2310.10631) / WizardMath: Continues pretraining CodeLlama 7B & 34B on Proof-Pile2.
    6. **DSP LEGOProver Lyra**: Relies on formal external prover tools. However, real-world math formulations may not have well-known informal proofs.

-   Datasets
	![image](https://github.com/user-attachments/assets/cd666ad0-23f1-4cd0-bde8-df38234dff49)

    1. **[AILAB] LEAN-GitHub**: Compiles GitHub Lean repositories for a versatile Lean prover (InternLM2-StepProver data). A large-scale Lean dataset (0.1B tokens) leveraging open-source Lean repositories on GitHub.
    2. **[AILAB] Lean Workbook**: A large-scale Lean problem set formalized from natural language math problems (previous work by the same team).

- Benchmark
	![image](https://github.com/user-attachments/assets/ae781ba6-db47-48ed-8748-d7327f82a998)

    1. **miniCTX**: Neural Theorem Proving with (Long-)Contexts.
    2. **FIMO**: [arxiv.org/abs/2309.04295](https://arxiv.org/abs/2309.04295).

-   Survey [(To 2024.4)](https://arxiv.org/pdf/2404.09939)
